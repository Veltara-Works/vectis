...
func GenerateKey(basePath, domain, selector string) (*KeyPair, error) {
	...
	// Write private key to disk. Modes are 0755 (dir) and 0644 (key) so the
	// rspamd container's unprivileged worker (uid 100 in the rspamd image)
	// can read keys generated by the api container (root). The volume is
	// internal to the Vectis stack — both api and rspamd are inside the
	// same trust boundary — so world-readable inside the volume is
	// acceptable. See feedback memory: outbound DKIM signing was silently
	// broken on prod 2026-05-03 because keys were 0600 root and rspamd
	// (uid 100) could not stat them.
	keyDir := filepath.Join(basePath, domain)
	if err := os.MkdirAll(keyDir, 0755); err != nil {
		return nil, fmt.Errorf("create DKIM key directory: %w", err)
	}
	if err := os.Chmod(keyDir, 0755); err != nil {
		return nil, fmt.Errorf("chmod DKIM key directory: %w", err)
	}

	keyPath := filepath.Join(keyDir, selector+".key")
	if err := os.WriteFile(keyPath, privPEM, 0644); err != nil {
...
// RepairPerms walks basePath and ensures DKIM key files and directories are
// readable by non-root users (rspamd runs as uid 100 inside its container).
// Idempotent — safe to call on every api startup. Existing installs created
// before the perm fix had keys at 0600 root, invisible to rspamd, which
// silently broke outbound DKIM signing.
func RepairPerms(basePath string) error {
	info, err := os.Stat(basePath)
	if err != nil {
		if os.IsNotExist(err) {
			return nil
		}
		return fmt.Errorf("stat DKIM base: %w", err)
	}
	if !info.IsDir() {
		return fmt.Errorf("DKIM base %s is not a directory", basePath)
	}
	return filepath.Walk(basePath, func(path string, fi os.FileInfo, err error) error {
		if err != nil {
			return err
		}
		var want os.FileMode = 0644
		if fi.IsDir() {
			want = 0755
		}
